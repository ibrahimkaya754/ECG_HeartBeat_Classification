...
########################## Read MIT Data Files created as .mat files ##########################################
random.seed(654)
def read_mitbih(filename, max_time=100, classes= ['F', 'N', 'S', 'V', 'Q'], max_nlabel=100, trainset=1):
    ...
    data = _data[:int(len(_data)/ max_time) * max_time, :]     ### int sonradan eklendi, orijinal repoda int
    _labels = _labels[:int(len(_data) / max_time) * max_time]  ### int sonradan eklendi, orijinal repoda int yok
...
def build_network(inputs, dec_inputs,char2numY,n_channels=10,input_depth=280,num_units=128,max_time=10,bidirectional=False):
    _inputs = tf.reshape(inputs, [-1, n_channels, int(input_depth / n_channels)])      ### int sonradan eklendi, orijinal repoda int yok
...
def run_program(args):
    ...
    X_train = X_train[:int(X_train.shape[0]/max_time)*max_time,:]         ### int sonradan eklendi, orijinal repoda int yok
    y_train = y_train[:int(X_train.shape[0]/max_time)*max_time]           ### int sonradan eklendi, orijinal repoda int yok
